...
const App = () => {
  ...
  const fetchMinecraftData = async (versionPath: string, idCategory: IDCategory) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`无法获取数据 (${response.status}): ${targetUrl}`);
      }
  ...
  const idCategories: IDCategory[] = ['全部', '物品与方块', '实体', '状态效果', '生物群系'];
  ...
  return (
    <div className="min-h-screen p-4 md:p-8">
          ...
          <section className="mc-panel p-4">
            ...
            {activeView === 'ids' && (
              <>
                ...
                <div className="mb-6">
                  <label className="text-xs text-gray-400 block mb-2 uppercase font-bold tracking-widest">ID 分类</label>
          ...
          {activeView === 'wiki' ? (
            ...
            <>
              <div className="mc-panel p-4 mb-4 bg-blue-900/20 border-blue-500/50 flex justify-between items-center">
                <div>
                  <h2 className="text-xl mc-font text-blue-400 mb-1">{selectedIDCategory === '全部' ? '物品与方块' : selectedIDCategory} ID 库</h2>
                  {fetchError ? (
                    <p className="text-xs text-red-400">⚠️ {fetchError} (正在显示本地兜底数据)</p>
                  ) : (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                {filteredIDs.length > 0 ? filteredIDs.slice(0, 450).map((item, idx) => (
                  ...
                  !isLoadingIDs && <div className="col-span-full mc-panel p-16 text-center text-gray-500">此版本或分类暂无数据</div>
                )}
                {filteredIDs.length > 450 && <div className="col-span-full p-4 text-center text-gray-500 text-xs">... 还有 {filteredIDs.length - 450} 个项，请通过搜索精确查找</div>}
              </div>
            </>
          )}
        </main>
      ...
      <footer className="max-w-6xl mx-auto mt-16 pt-8 border-t border-gray-800 text-center text-gray-500 text-sm pb-8">
        ...
        <p className="mt-1">数据源: PrismarineJS / minecraft-data • 实时同步</p>
      </footer>
    </div>
  );
};
